...
The commercial SEE is a separate one-time product at **A$749 before credits**. Protected non-production contracts now include deterministic real DOCX/PDF rendering for accepted final candidates and visibly qualified working outputs. The renderer carries exact project/evidence provenance into document control, current-issue revision history, supporting-evidence and source schedules, page-numbered DOCX/PDF structure, and preserves the working/final readiness boundary. A same-scope paid A$49 Planning Controls Pack earns one single-use A$49 SEE credit, producing an A$700 balance for that exact project/site/QSC/proposal scope. The credit is not transferable, reusable, cash-redeemable, or proof of planning readiness. **Production SEE checkout/export is still disabled**, and professional visual parity is not yet accepted: representative DOCX/PDF outputs must still be rendered and compared against the approved SEE benchmark library before the launch document-quality gate can close.
