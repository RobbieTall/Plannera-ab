...
Item 72C adds a protected, manual-only Stripe test-mode acceptance workflow and [operator runbook](docs/operations/stripe-test-mode-acceptance.md). The runner verifies provider/session facts for the Australian A$49.00 total and A$4.45 GST, signed-webhook settlement, exact entitlement, negative cross-scope cases, duplicate Checkout denial, and three-phase before-payment/paid/full-refund reconciliation while emitting only safe results and opaque IDs. The Stripe-hosted test payment remains an explicit manual operator step; a redirect cannot produce a green result. This acceptance infrastructure does not configure Stripe, GitHub, Vercel, or production and has not yet been executed. Code deployed remains distinct from checkout enabled; Production must keep `PLANNING_PACK_CHECKOUT_ENABLED` false/absent until a separate explicitly approved activation PR.
