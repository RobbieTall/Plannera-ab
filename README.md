...
Item 74 evidence intake is in progress. New uploads retain a SHA-256 source hash and explicit extraction/readability and indexing states. PDF page references, DOCX text, XLSX sheets, CSV and plain text can enter the project source index; parser warnings, unsupported legacy formats and indexing failures remain visible and cannot silently support an SEE. Issue #437 adds a provider-neutral OCR lifecycle for image-only/scanned evidence: source-hash-bound queue/retry attempts, provider-result integrity, mandatory visual review, and promotion through the existing indexing path only after approval. No live OCR provider is connected and unreviewed OCR text remains outside retrieval. Structured map/plan provenance is already materially implemented with source/date/layer/legend/observation/limitation rather than remaining wholly open. The protected Item 74H chain still requires four independently reviewed private roles: current road classification, registered cadastral plan, detail survey reconciled to that plan, and proposed layout bound to the survey. Production checkout remains disabled. Real registered-plan execution, live OCR-provider acceptance, residual conflict/freshness reconciliation and final commercial activation remain open gates.
