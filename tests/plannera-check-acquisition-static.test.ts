...
test("focused Check promotion is a single same-project artefact save", () => {
  ...
  assert.match(workspace, /!focusedCheckResult \|\| isPromotingCheck \|\| promotedCheck \|\| !hasLoadedServerArtefacts/);
  assert.match(workspace, /disabled=\{!hasLoadedServerArtefacts \|\| !focusedDevelopmentIntent\.trim\(\) \|\| isPromotingCheck \|\| promotedCheck\}/);
