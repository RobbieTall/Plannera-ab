...
- The Vercel Preview status for that head is still red before application build. Read-only Neon inspection proved the database is at its 10/10 branch limit: `main`, the active protected `preview/ops/stripe-test-acceptance`, and eight idle historical Preview branches. No branch has been deleted. One proven-stale merged Preview branch must be removed with explicit operator approval, then #333 must be redeployed and its migration/build inspected before merge. Do not infer deployment readiness from the green application gate.
