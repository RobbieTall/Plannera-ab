...
- The protected runner and evidence path are now operational. The existing sandbox Checkout no longer satisfies the required unpaid/open provider phase, consistent with an expired or otherwise non-open fixture. Plannera still reports the purchase as `waiting`; the production UI intentionally hides retry while that durable state remains pending. Recover through the real Stripe `checkout.session.expired` webhook/reconciliation path before creating one fresh normal-product sandbox Checkout. Do not patch entitlement data directly and do not create duplicate sessions casually.
- Next action: verify or resend the existing test-mode expiration event to the protected preview webhook so the exact purchase becomes `cancelled`; confirm the workspace offers `Continue to secure checkout`; create exactly one fresh Checkout through that normal UI; replace only the protected opaque Checkout Session ID; then redispatch `before_payment`.
