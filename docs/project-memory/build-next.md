...
- The first operator magic-link attempt reached the old one-off deployment URL without a token and failed closed with `Missing token`. It created no authenticated requester, project, Checkout Session, payment, DPP, charge, refund, or protected acceptance run.
- Next action: use the prepared sign-in form on the stable branch alias, send and open one fresh magic link, verify the dedicated requester is signed in, then create two disposable isolated projects and dispatch `before_payment`.
