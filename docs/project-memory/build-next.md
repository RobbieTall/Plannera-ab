...
Remaining before 74A is complete: add an asynchronous OCR/provider path with operator-visible retry/review; add document applicability/acceptance for successfully indexed uploads; reconcile upload/spatial facts against the proposal and statutory chain beyond the explicit spatial conflict state; and make the final SEE section readiness contract consume only readable, indexed, accepted and conflict-free evidence.
