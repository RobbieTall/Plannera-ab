...
#### Item 72C execution checkpoint — IN PROGRESS; BEFORE-PAYMENT CONFIGURATION HANDOFF (2026-07-27)

- PR #324 merged to `main` at exact commit `707759763365dd86186a2b3f89727885cf7f2056`. The merged two-file fix resolves an owned project through canonical ID or public `proj-*` ID, keeps the requester ownership condition, canonicalises durable purchase scope to the returned internal project ID, and adds the public-ID regression.
- The Commercial Funnel Golden Gate and Vercel preview passed for reviewed #324 head `cfb31f7694cb4f4e6f7112ef933077541707ce87`.
- Git branch `ops/stripe-test-acceptance` is repinned to exact merged #324 commit `707759763365dd86186a2b3f89727885cf7f2056`. Its Vercel Preview deployment is Ready and the stable branch alias remains `https://plannera-ab-git-ops-stripe-test-acceptance-robbietalls-projects.vercel.app`.
- The stable alias retained the dedicated authenticated requester and disposable scopes. Primary Byron project `proj-72c39c6b5cf645ca9b` again opens normally with the current cited SP3 Quick Site Check and fixed `Tourist and visitor accommodation` proposal; independent Kempsey project `proj-892dfa6e434242fb8d` remains the negative scope.
- Exactly one Stripe sandbox Checkout Session was created through the normal signed-in Byron workspace after the corrected deployment. It is still open/unpaid. Returning from Checkout leaves the application at exact `state=waiting`; no redirect or hosted URL granted entitlement.
- No card or Australian billing data has been entered. No payment, webhook settlement, DPP, charge, refund, protected workflow run, or safe acceptance artifact exists.
- The protected GitHub environment `stripe-test-acceptance` retains required-reviewer/main-only protection. All five variables and all five secrets are now configured, including the operator-entered session cookie header and Stripe test key. Secret values remain encrypted and out of docs/logs; their validity has not yet been proven because `before_payment` has not been dispatched.
- On 2026-07-31, a read-only query against the exact isolated Neon Preview branch `preview/ops/stripe-test-acceptance` (`br-restless-sea-a7iphknt`) returned the single Byron purchase row as application status `PENDING`; its opaque QSC artefact ID was entered directly into the protected GitHub environment variable. No database row was modified. The Stripe provider session state still requires runner verification and may have expired since creation.
- PR #325 repaired the YAML line-77 parse blocker and merged to `main` at exact commit `60fb57920d99d0aee9ce50d19964d8c6cd7f63d2`. Protected `before_payment` workflow run `30596022031` (workflow sequence 22) was dispatched and approved on that exact commit. Checkout, Node setup and lifecycle-script-free dependency installation passed, but the acceptance step then failed before its first Stripe or application assertion because Prisma Client had not been generated after `npm ci --ignore-scripts` (`Cannot find module '.prisma/client/default'`). No provider/application acceptance request, payment, DPP, refund or safe artifact occurred. PR #326 added the database-free Prisma generation plus ordering regression, passed Commercial Funnel Golden Gate run `30598123330` and Vercel at exact head `fc740585fd9a2bf913881575e3b9f691e47671ee`, and merged to `main` at exact commit `c44a3d9995411bfaaf7aebf557db86cbb103a73f`. Protected `before_payment` rerun `30598962753` (workflow sequence 26) was approved and executed on that exact merge commit. Prisma generation passed and the runner reached the real Stripe/application acceptance logic, which returned non-zero. Because the workflow stopped immediately at that step, safe-summary validation, job summary, and artifact upload were skipped; therefore the allowlisted failure reason was not retained and must not be guessed. No payment, webhook settlement, DPP, refund, production activation, or safe artifact occurred.
- PR #327 is the narrow failure-evidence repair. It permits the acceptance command to finish without stopping later evidence steps, validates the privacy-minimal pass-or-fail schema, writes only safe phase/passed/reason fields, uploads the safe artifact, and then still fails the job whenever `passed !== true`. Its exact head `23bec0a967c1afae21fecfd999530003624d3b39` passed Commercial Funnel Golden Gate run `30599825597` and its Vercel preview. It does not weaken the gate or create/modify a payment, entitlement, DPP, refund, or production setting.
- PR #327 merged to `main` at exact commit `2f896a1be00e7ade83152fed5a2880ce14b8e3c8`. Protected `before_payment` run `30600253122` (workflow sequence 29) was dispatched on that exact commit and approved. Checkout, dependency installation, Prisma generation, and the acceptance runner all executed. The runner returned non-zero as designed, but `npm run` prepended its normal `> plannera-app...` script banner to redirected stdout, so the privacy-minimal file was not valid JSON and validation failed before summary/upload. The safe reason was therefore still not retained and must not be inferred. No payment, webhook settlement, DPP, refund, production activation, or safe artifact occurred.
- PR #328 is the pure-JSON capture repair: invoke the unchanged runner with `npm run --silent accept:stripe-test`, update the ordering regression to that exact command, and reject restoration of the non-silent command. Exact head `707428045d80ee058275760dfa6f3c1c7ee675c3` passed Commercial Funnel Golden Gate run `30601288934` and Vercel. It does not change acceptance logic, reason taxonomy, or the final fail-closed enforcement.
- PR #328 merged to `main` at exact commit `eb6528c8bbeac74aa4f689b67ad2470ac02ffa52`. Protected `before_payment` run `30602326972` (workflow sequence 31) was dispatched on that exact commit and approved. Silent runner capture produced valid JSON and reached the privacy-minimal schema validator. Validation failed because actual check keys were sorted before comparison while the manually listed expected keys began `checkoutReplaySafe, changedScopesDenied` instead of alphabetical `changedScopesDenied, checkoutReplaySafe`. No safe reason/artifact was retained; no payment, webhook settlement, DPP, refund, or production activation occurred.
- PR #329 repaired the exact schema-order defect by sorting both expected exact-key arrays before comparison while retaining rejection of missing or extra fields. Exact head `9247889e7c8a368143a850f8133d186f9af153b7` passed Commercial Funnel Golden Gate run `30604269577` and Vercel, then merged to `main` at exact commit `a1ef0a7a0a46040cddcf90210b8482f64e8d5629`. Deleting its source branch after merge was normal cleanup and lost no code.
- Protected `before_payment` run `30604987325` (workflow sequence 33) was dispatched and operator-approved against exact merged commit `a1ef0a7a0a46040cddcf90210b8482f64e8d5629`. Dependency install, database-free Prisma generation, the real provider/application runner, exact privacy-schema validation, safe job summary, and safe artifact upload all succeeded. Final fail-closed enforcement correctly left the job red because the validated result was `phase=before_payment`, `passed=false`, `reason=phase_mismatch`. Safe artifact `stripe-test-acceptance-summary` has ID `8783557614`, SHA-256 `0943187a5c831262ff2328ff9607426521bcb4ed201c44581cdd5ba4d731358c`, and expires 2026-08-14. No raw provider response, proposal, address, contact, card, cookie, key, or secret was retained.
- The protected runner and evidence path are now operational. Investigation found Stripe had generated the exact `checkout.session.expired` event for the fixture, but all three delivery attempts were rejected with HTTP 401 by Vercel Authentication before Plannera received them. The application webhook itself returns 400 for signature failure, so the 401 conclusively identified deployment protection rather than payment-code failure.
- A dedicated Vercel Protection Bypass for Automation secret was created with note `Stripe sandbox webhook - Item 72C`; its value remains secret and is not recorded in GitHub, artifacts, logs, or documentation. Only the Item 72C Stripe sandbox destination was updated to carry the bypass query parameter. The Production Stripe destination and Production deployment protection were untouched.
- The existing exact `checkout.session.expired` sandbox event was resent after the destination repair. The protected Byron workspace then moved from `waiting` to `cancelled` and displayed `Checkout was cancelled or expired. You can try again.` plus `Continue to secure checkout`. This proves the real signature-verified expiration webhook reached Plannera and applied the guarded durable lifecycle transition. No database row was patched directly.
- With explicit operator approval, exactly one fresh unpaid Checkout was created through the normal Byron workspace UI. Its opaque ID is `cs_test_a1bytRT9icJsz3C8uVaYHtmd5ugQkStZpjOYEpsCER9ZBAtJZUGFLaIrac`; Stripe-hosted Checkout remains open and untouched, with no card or billing data entered.
- Protected `before_payment` run `30608220299` executed against exact merged `main` commit `a1ef0a7a0a46040cddcf90210b8482f64e8d5629`. Configuration, Stripe test mode, unpaid/open provider phase, fixed A$49 AUD terms and complete unique-session pagination all passed. The privacy-minimal summary and artifact upload also passed, then final enforcement correctly failed closed with `reason=application_request_failed` before any application-scope or DPP assertion.
- Safe artifact `stripe-test-acceptance-summary` has ID `8784634379`, SHA-256 `b004c43dee60f1d2bc3f353f4dd6717dc7d1b202649eafedbcddbcc421909492`, and expires 2026-08-14. It contains only the allowlisted schema and opaque Checkout ID; no provider payload, proposal, address, contact, card, cookie, key or bypass value.
- Root cause: the Stripe sandbox webhook destination now carries its dedicated Vercel automation bypass, but the GitHub acceptance runner's direct requests to protected Plannera endpoints did not. PR #330 is the narrow repair: require a dedicated runner value from the protected GitHub environment, send it only as the `x-vercel-protection-bypass` header on every Plannera application request, keep provider calls and safe output unchanged, and test propagation/redaction.
- PR #330 exact head `476ee7748fff167ba2eac36d9a15fe92c9abe817` passed Commercial Funnel Golden Gate run `30609122621` and its Vercel preview. The diff is limited to the runner, its targeted tests, and protected workflow secret injection; no local build was used. The operator then merged #330 to `main` at exact commit `ed9b8e331b2e261aa4a4fa7386e299d6f22a9224`.
- A separate 32-character Vercel Protection Bypass for Automation value was generated with note `GitHub acceptance runner - Item 72C` and transferred directly into the protected GitHub environment as secret `PLANNERA_STRIPE_TEST_VERCEL_BYPASS`. Its value is not recorded in chat, documentation, logs or artifacts. The existing Stripe webhook bypass remains separate.
- Protected `before_payment` run `30609965167` was dispatched exactly once from merged `main` using the existing fresh open/unpaid Checkout and operator-approved. Dependency install, Prisma generation, the real runner, privacy validation, safe summary and artifact upload all succeeded. Final enforcement failed closed with `reason=application_request_failed` after Stripe configuration, test mode, unpaid/open provider phase, fixed A$49 AUD terms and complete unique-session pagination passed.
- Safe artifact `stripe-test-acceptance-summary` has ID `8785649399`, SHA-256 `3ec05c9b802acbd99af58b2b7689717317b5822582fc0fbbb74c1b82ee01b87d`, and expires 2026-08-14. It retains only the allowlisted summary and opaque Checkout ID.
- Vercel's exact request trace at 2026-07-31 17:04:09 AEST shows the runner POST to `/api/planning-pack/status`: firewall `Allowed`, Next.js function invocation executed for 700 ms, then the application returned HTTP 401. This proves the new runner bypass reached the application and isolates the remaining failure to requester-session authentication, not Stripe or Vercel protection.
- The exact preview workspace still renders an authenticated `Logout` state in Chrome, so a valid browser session exists. The operator copied the exact stable-preview session token value without placing it in chat; the protected GitHub `PLANNERA_STRIPE_TEST_SESSION_COOKIE` was replaced in full header form `__Secure-next-auth.session-token=<redacted>`. No cookie value is retained in documentation, logs or artifacts.
- Corrected protected `before_payment` run `30613249161` was dispatched exactly once on the same merged release and existing open/unpaid Checkout, then operator-approved. Dependency install, Prisma generation, the real runner, privacy validation, safe summary and artifact upload all succeeded. Final enforcement failed closed with `reason=application_request_failed`: Stripe configuration, test mode, unpaid/open provider phase, fixed A$49 AUD terms and complete unique-session pagination passed, while the first application-scope request still failed before any scope or DPP assertion.
- Safe artifact `stripe-test-acceptance-summary` has ID `8786351613`, SHA-256 `bcd096a3e88e686864d25dadc8beefd3739b82ecf34d3d94995d0e998c8856d6`, and expires 2026-08-14. It retains only the allowlisted summary and opaque Checkout ID.
- The same exact protected preview workspace continues to render an authenticated `Logout` state and the expected unpaid `waiting` pack state in Chrome. Source inspection confirms the application expects `__Secure-next-auth.session-token` in secure deployments. Therefore Stripe, the fixture, browser authentication and Vercel bypass are healthy; the remaining boundary is the GitHub runner's stored/forwarded requester credential. Do not pay or dispatch another acceptance run until that credential path is repaired and independently verified.
- On 2026-08-02 the operator refreshed `PLANNERA_STRIPE_TEST_SESSION_COOKIE` from the exact stable preview and attempt 2 of protected run `30613249161` was started exactly once and approved. The workflow plumbing, privacy validation and safe artifact upload passed; final enforcement failed closed with `reason=phase_mismatch` before any application request because the previously open Checkout had expired during the two-day pause. Provider phase, terms and pagination therefore remained false in this attempt, and the refreshed requester credential was not exercised or proven.
- Attempt-2 safe artifact `stripe-test-acceptance-summary` has ID `8828218050`, SHA-256 `0942156d535db68d53f836d58899beade1523a59bd882743717cd3a7699a68a4`, and expires 2026-08-16. No card/billing data, payment, entitlement, DPP or refund exists. The expired Checkout must not be reused; creating exactly one fresh unpaid Checkout now requires explicit operator approval.
- With explicit operator approval on 2026-08-02, exactly one new unpaid A$49 Stripe sandbox Checkout was created through the normal authenticated protected Byron workspace. The hosted Checkout is open and untouched; no email, phone, card, billing data or payment was entered. Its identifier is intentionally excluded from repository documentation. The Checkout identifier was supplied only as the protected workflow's required `checkout_session_id` dispatch input; it was not added to the environment or repository documentation. Protected `before_payment` run `30732647096` was submitted exactly once against merged `main` commit `ed9b8e331b2e261aa4a4fa7386e299d6f22a9224`, operator-approved, and passed. Configuration, Stripe test mode, open/unpaid provider phase, fixed A$49 AUD terms, unique-session pagination, checkout replay safety, exact-scope application status, changed-proposal and cross-project denial, unpaid DPP gate, and terminal-state expectations all passed; no DPP was created. Safe artifact `8828482459` has SHA-256 `b2b1ede429a9df6dab6d11b82fce59cda3c14fbc3440b8d36b72121f46e7e684` and expires 2026-08-16. The before-payment gate is complete; the next operator-only action is one manual Stripe-hosted sandbox payment with test data, followed by webhook settlement verification and a protected `paid` run.
- The operator then completed that one Stripe-hosted sandbox payment using synthetic Australian test data. Stripe test mode recorded one succeeded A$49.00 AUD payment with A$4.45 GST, and the protected Plannera workspace exposed the exact-scope paid Detailed Planning Pack path, independently confirming that provider settlement and application entitlement had both landed. No Production checkout, key, destination, deployment, charge or customer data was touched.
- Protected `paid` run `30733131476` (workflow sequence 50) was dispatched exactly once against the same merged `main` commit and paid Checkout, then operator-approved. Provider phase/terms, unique-session pagination, checkout replay safety, exact-scope paid status, and changed-proposal/cross-project denial all passed. The runner successfully created exactly one DPP (`dppCreatedThisRun=true`) but then failed closed with `reason=dpp_gate_failed`; safe artifact `8828631228` has SHA-256 `ecb473cde66e792f4bbc767407ddb23f6ac1b69195c846a0bf8273ea9c412c5f` and expires 2026-08-16. Root-cause inspection found a verifier-only identity mismatch: the project-scoped artefact endpoint is addressed by the public `proj-...` identifier, while the persisted DPP payload correctly stores the internal database project ID. The test harness had incorrectly made those values identical. The product purchase/entitlement path and DPP creation succeeded; Item 72C remains incomplete until the verifier is repaired and `paid` reruns green.
- A subsequent protected-workspace refresh rendered the fixture project with `No site set` and zoning unavailable despite the same page showing a valid Byron site immediately before the paid run and the DPP creation request requiring a valid site context. The acceptance runner contains no site mutation. Treat this as an unresolved fixture/deployment/rendering observation: verify persisted site context and the current QSC/DPP chain before relying on the fixture for the final paid/refunded proof.
- Focused verifier repair PR #331 passed Commercial Funnel Golden Gate run `30733798586`. After the explicitly approved deletion of only the idle Neon Preview branch for merged PR #324, post-cleanup Preview deployment `9wPXYx9zZEAZqtrBDcC1xRNBWRLh` provisioned Neon and completed `Ready`. PR #331 then merged to `main` as `ec8875138462c50b98da2151b2ba3be1e86d0259`.
- The protected Byron fixture was reverified after merge: 45 Broken Head Road persisted as SP3 Tourist; the exact “Tourist and visitor accommodation” QSC remained current; and the paid A$49 DPP rendered the same cited and unresolved DCP topic chain. This resolves the earlier missing-site rendering observation for the current fixture.
- Protected `paid` rerun `30734666290` was dispatched exactly once on merged `main`, approved for the protected non-production environment, and passed: `phase=paid`, `passed=true`, `reason=none`. Safe artifact `8829151091` has SHA-256 `41cf7740a883d1c5cb383121ec2d776ada4dfeae48a328487cef87668cd441d2` and expires 2026-08-16.
- With explicit operator approval on 2026-08-02, exactly one full A$49.00 AUD refund was issued against the same dedicated Stripe sandbox payment. Stripe recorded that one payment as `Refunded`; the other eight sandbox payments were untouched. The protected Byron workspace retained the project, SP3 site and exact-proposal Quick Site Check while displaying `This payment was refunded and pack access has been removed.` plus a new secure-checkout path. This independently proves the real refund webhook reversed only the exact-scope entitlement without deleting the project evidence chain.
- Protected `refunded` run `30735071908` (workflow sequence 57) was dispatched exactly once against merged `main` commit `ec8875138462c50b98da2151b2ba3be1e86d0259`, operator-approved for the protected `stripe-test-acceptance` environment, and passed: `phase=refunded`, `passed=true`, `reason=none`.
- Safe artifact `8829295014` has SHA-256 `bdff979152e7b14d6993566bc4960e9014e48033d4112f11803a92de0a8ad015` and expires 2026-08-16. No raw provider response, proposal, address, contact, card, cookie, key, bypass value or secret was retained.
- Production checkout remains disabled; no production environment, webhook, key, payment, project, or production data was changed.

Item 72C is complete with nine validated privacy-safe artifacts. `before_payment`, `paid` and `refunded` passed on the same dedicated Checkout. The real sandbox payment, A$4.45 GST, webhook-backed exact-scope entitlement, replay and scope isolation, one DPP creation, full refund, and webhook-backed entitlement reversal are proven. Run `30733131476` exposed the verifier-only public/internal project-ID mismatch, PR #331 repaired it, and runs `30734666290` and `30735071908` passed the corrected paid and refunded gates on merged `main`. No Production activation occurred. The next build priority is Item 73's real consultant referral submission and truthful delivery-state contract.
