...
- Do not refund yet. First merge the narrow verifier repair, verify the protected fixture's persisted site context and artefact provenance, then rerun `paid` against the same paid Checkout. Only after a green paid artifact should the operator perform one full A$49 test refund and dispatch `refunded`.
