...
- The protected GitHub environment `stripe-test-acceptance` retains required-reviewer/main-only protection. All five variables are now configured: the exact/allowed stable Preview URL, both public project IDs, and the primary QSC artefact ID. Three of five secrets are configured: the primary proposal, changed-proposal negative scope, and exact two-field DPP request JSON. Values remain out of docs/logs. `PLANNERA_STRIPE_TEST_SESSION_COOKIE` and `STRIPE_TEST_SECRET_KEY` remain absent, so `before_payment` has not been dispatched.
...
- Next action: the operator must add the existing test Stripe key and a fresh full `__Secure-next-auth.session-token=<value>` cookie header directly as the two remaining GitHub secrets without pasting either into chat. Then reassess the existing `cs_test_*` session through `before_payment`; if Stripe reports it expired, create exactly one fresh normal-product sandbox Checkout and rerun `before_payment`. The run must prove open/unpaid provider state, exact application `waiting`, one-session uniqueness, negative-scope isolation, and HTTP 402 DPP denial before any manual card action.
