...
- The protected GitHub environment `stripe-test-acceptance` still has the required reviewer/main-only protection but currently contains no acceptance variables or secrets. Therefore `before_payment` has not been dispatched.
- Next action: obtain the opaque current QSC artefact ID through a protected read-only surface, populate the five documented environment variables and five secrets without exposing their values, then dispatch `before_payment` with the one existing `cs_test_*` session. The run must prove open/unpaid provider state, exact application `waiting`, one-session uniqueness, negative-scope isolation, and HTTP 402 DPP denial before any manual card action.
