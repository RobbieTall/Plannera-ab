...
#### Item 72C execution checkpoint — IN PROGRESS; DEDICATED REQUESTER SIGN-IN HANDOFF (2026-07-26)

- Git branch `ops/stripe-test-acceptance` remains pinned to exact merged #322 commit `1cc0b21c2bb01b2bb8e47bbc5ab5ae82a26e372c`.
- Protected GitHub environment `stripe-test-acceptance` requires reviewer `RobbieTall`, disables administrator bypass, and permits only `main`. It still contains no acceptance variables or secrets; those are populated only after disposable requester/project scope exists.
- Vercel-managed Neon Preview database branching is enabled. Production database branching remains off.
- The isolated branch preview has been rebuilt successfully from exact commit `1cc0b21` with the stable branch alias `https://plannera-ab-git-ops-stripe-test-acceptance-robbietalls-projects.vercel.app`; the latest deployment is Ready. Production was not redeployed or reconfigured.
- Vercel now contains exactly the required acceptance configuration: `PLANNING_PACK_CHECKOUT_ENABLED`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `PLANNING_PACK_CHECKOUT_SUCCESS_URL`, `PLANNING_PACK_CHECKOUT_CANCEL_URL`, `NEXT_PUBLIC_AUTH_ENABLED`, `APP_URL`, and `NEXTAUTH_URL`. Every row is Sensitive, Preview-only, and scoped only to `ops/stripe-test-acceptance`. All base/callback URLs use the stable branch alias so redeploys cannot strand auth or payment callbacks.
...
- Stripe Tax sandbox configuration has an Australian standard GST registration, the `Professional Services` preset product tax code, 10% GST, and automatic tax enabled/marked complete. The A$49.00/A$4.45 provider fact is not yet proven because no Checkout Session exists.
- The active test webhook destination now uses the stable branch endpoint `https://plannera-ab-git-ops-stripe-test-acceptance-robbietalls-projects.vercel.app/api/webhooks/stripe` and subscribes exactly to `checkout.session.completed`, `checkout.session.async_payment_succeeded`, `checkout.session.async_payment_failed`, `checkout.session.expired`, `charge.refunded`, `refund.created`, and `refund.updated`. It has no deliveries.
- The first operator magic-link attempt reached the old one-off deployment URL without a token and failed closed with `Missing token`. After the stable callback correction and redeploy, a fresh magic link succeeded and the dedicated requester is now authenticated on the stable branch alias.
- Two disposable isolated scopes now exist under that requester: primary Byron project `proj-72c39c6b5cf645ca9b` with a current cited SP3 Quick Site Check and fixed `Tourist and visitor accommodation` proposal, plus independent Kempsey project `proj-892dfa6e434242fb8d` with a current cited E2 Quick Site Check and fixed `Commercial premises` proposal.
- The first primary Detailed Planning Pack attempt failed closed with `Project not found` before any Checkout Session. Acceptance exposed that the workspace sends the public `proj-*` route identifier while `PurchaseEntitlementService.resolveScope` queried only canonical `Project.id`.
- Draft PR #324 (`fix/item-72c-public-project-scope`, head `cfb31f7694cb4f4e6f7112ef933077541707ce87`) resolves an owned project through canonical ID or public ID, canonicalises the durable purchase scope to the returned internal ID, and adds a public-ID regression test. Its Commercial Funnel Golden Gate and Vercel preview are pending.
...
- Next action: require green remote checks and operator-approved merge/deploy of PR #324, repin the isolated acceptance branch/deployment to that exact corrected release, then resume with one Checkout and dispatch `before_payment`.
