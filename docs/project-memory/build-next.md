...
- The exact preview workspace still renders an authenticated `Logout` state in Chrome, so a valid browser session exists. The protected GitHub `PLANNERA_STRIPE_TEST_SESSION_COOKIE` is stale, from the wrong preview session, or incorrectly formatted. Next action: replace it from the exact stable preview using the full session cookie header form `cookie-name=value`, then redispatch `before_payment` exactly once with the same Checkout. Do not enter card or billing data until it passes.
