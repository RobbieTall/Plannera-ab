...
- Only after `before_payment` passes may the operator manually complete Stripe-hosted Checkout using the documented Australian test payment method/address. `paid` and `refunded` remain blocked until their exact provider/webhook facts exist.
...
Item 72C now has six validated privacy-safe artifacts and the `before_payment` phase has passed; `paid` and `refunded` remain incomplete. Runs `30596022031`, `30598962753`, `30600253122`, and `30602326972` exposed and drove narrow workflow repairs; run `30604987325` isolated the expired fixture; run `30608220299` isolated the GitHub-runner Vercel protection boundary; runs `30609965167` and `30613249161` proved the bypass reaches the application and isolated the still-unresolved GitHub-runner requester-session credential path. No payment, webhook-settled entitlement, DPP, refund, or production activation occurred.
