...
- The protected GitHub environment `stripe-test-acceptance` retains required-reviewer/main-only protection. Four of five variables are now configured: the exact/allowed stable Preview URL and both public project IDs. Three of five secrets are configured: the primary proposal, changed-proposal negative scope, and exact two-field DPP request JSON. Values remain out of docs/logs. `PLANNERA_STRIPE_TEST_QSC_ARTEFACT_ID`, `PLANNERA_STRIPE_TEST_SESSION_COOKIE`, and `STRIPE_TEST_SECRET_KEY` remain absent, so `before_payment` has not been dispatched.
- Next action: complete Vercel's identity check for the read-only database query, obtain only the opaque current QSC artefact ID and add its final variable. The operator must add the existing test Stripe key and full `__Secure-next-auth.session-token=<value>` cookie header directly as the two remaining GitHub secrets without pasting either into chat. Then dispatch `before_payment` with the one existing `cs_test_*` session. The run must prove open/unpaid provider state, exact application `waiting`, one-session uniqueness, negative-scope isolation, and HTTP 402 DPP denial before any manual card action.
