...
Implementation checkpoint — spatial evidence and review foundation (`feat/item-74-spatial-evidence`):
- Map snapshots are promoted from unverified screenshots to typed `SpatialEvidence` records bound to the exact confirmed site fingerprint, source authority/URL, image SHA-256 hash, selected layers, legend state, capture/effective/source-check dates, user-confirmed observation and explicit limitation. Government/council captures require a source URL; future dates, missing layers, unconfirmed observations and unexplained legend gaps fail closed.
- Spatial evidence expires 90 days after its source-check date. `Pending review`, `Accepted`, `Rejected`, `Conflict` and `Superseded` states are durable; accept is refused for stale evidence, unresolved legends or a changed site. Each decision appends a review event and uses optimistic versioning so concurrent decisions cannot silently overwrite one another.
- A deterministic readiness resolver returns only accepted, current-site, non-expired evidence and exposes pending review, conflict, staleness, site mismatch, legend and no-accepted-evidence blockers for the final SEE compiler. Legacy screenshots remain visible but are explicitly not evidence-ready.
- The map UI captures the full provenance contract and exposes truthful review controls/status. On 3 August 2026, exact-head Commercial Funnel Golden Gate run `30777837920` generated Prisma and passed 163 Node tests plus 77 Vitest tests with zero failures; Vercel Preview deployment `JBDk82xe8QLeitjWez9JVKfo2sDM` completed the additive migration and full application build successfully.

Implementation checkpoint — uploaded evidence applicability and acceptance (`feat/item-74-evidence-reconciliation`, PR #338 application head validated):
- Every upload now begins `Pending review` and records an exact planning-use decision as `Accepted`, `Rejected`, `Conflict` or `Superseded`. A review is bound server-side to the requester-owned project, current confirmed site fingerprint, exact normalized proposal fingerprint, exact Detailed Planning Pack and the real current Quick Site Check named by that pack; a client-supplied label or stale artefact cannot establish applicability. Accepted/conflicting evidence is also assigned to one or more finite SEE topics, so a report can support or block only the sections actually reviewed.
- Acceptance is refused unless the source is fully readable and successfully indexed. Document date is mandatory, future dates fail closed, optional validity dates must remain current, and optimistic versioning plus append-only review events prevent concurrent decisions from silently overwriting one another.
- Final-section readiness consumes only accepted, readable, indexed, non-expired evidence with exact current scope. Pending, conflicting, expired and cross-scope documents remain explicit blockers; rejected or superseded material stays in project history but cannot support or block a final SEE.
- The Sources panel separates technical readability from planning applicability, opens the immutable source independently, and provides a focused review surface for accept/conflict/not-applicable decisions against the current pack. Regression coverage includes exact acceptance, real QSC-chain resolution, unreadable/unindexed/future/wrong-site refusal, conflict notes, expiry/scope blockers and rejected/superseded exclusion.

Implementation checkpoint — cross-source SEE topic boundary (`feat/item-74-evidence-reconciliation`, PR #338 application head validated):
- Upload and spatial reviews use one finite, version-controlled set of SEE evidence topics covering site context, statutory planning, design, traffic, hazards, biodiversity, heritage, contamination/geotechnical, servicing/waste, amenity and public-interest effects. Accepted or conflicting evidence must name at least one topic; rejection/supersession carries no supporting topic.
- Spatial layer names remain immutable source metadata and are not heuristically promoted into document sections. Reviewers explicitly select the sections affected by the observation. Legacy accepted evidence without a topic fails closed until reviewed again.
- Topic assignments are stored on the current evidence record and copied into each append-only review event. This is the precondition for section-level readiness and conflict reconciliation: evidence may support or block only selected sections and cannot raise confidence across the whole SEE.
- On 3 August 2026, PR #338 application head `304bc4b7c416a99438ce5b643394bf782c57feed` passed Commercial Funnel Golden Gate run `30779827428`: Prisma generation, 173 Node tests and 77 Vitest tests, zero failures. Vercel Preview deployment `4bLkUzJTSpGhRiuH8UAj68geU1hU` completed both additive migrations and the full application build successfully. The earlier preview correctly rejected a redundant existing-table composite unique constraint; the implementation now uses Prisma's supported unique-ID-plus-version filtered write without accepting data loss.

Remaining before 74A is complete: add an asynchronous OCR/provider path with operator-visible retry/review; implement the section-level reconciliation ledger against the statutory chain; and connect topic conflict/freshness outcomes to the versioned SEE section-readiness contract.
