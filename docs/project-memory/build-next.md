...
Delivered on branch `codex/implement-item-64-workspace-funnel-navigator` from required base/head `bdc9c4a8023510a00faaae90e387650fd6d4962d` (final head placeholder: `<item-65-head>`). The homepage remains the real product entry with one labelled Site address field, one dominant **Run free site check** action, honest Byron/Kempsey pilot disclosure, and only the approved Byron/Kempsey launch examples. Submitting an address now opens the existing requester-scoped project workspace with a small `check=1` focused-mode query contract rather than a parallel app or backend.
