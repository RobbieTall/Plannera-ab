...
- Do not refund without separate explicit operator approval. The next and final Item 72C gate is exactly one full A$49 sandbox refund for the same Checkout, webhook-backed entitlement reversal verification, and one protected `refunded` run. Do not create another Checkout and do not touch Production.
...
Item 72C now has eight validated privacy-safe artifacts. `before_payment` and `paid` have passed on the same dedicated Checkout; the real sandbox payment, webhook-backed exact-scope entitlement, replay/scope isolation and one DPP creation are proven. Run `30733131476` exposed the verifier-only public/internal project-ID mismatch, PR #331 repaired it, and run `30734666290` passed the corrected paid gate on merged `main`. `refunded` has not started. No refund or Production activation occurred.
