...
- Root cause: the Stripe sandbox webhook destination now carries its dedicated Vercel automation bypass, but the GitHub acceptance runner's direct requests to protected Plannera endpoints did not. PR #330 is the narrow repair: require the same dedicated value from the protected GitHub environment, send it only as the `x-vercel-protection-bypass` header on every Plannera application request, keep provider calls and safe output unchanged, and test propagation/redaction.
- Next action: after PR #330 checks pass and the operator explicitly approves its merge, add the existing dedicated bypass value as protected environment secret `PLANNERA_STRIPE_TEST_VERCEL_BYPASS`; then redispatch `before_payment` exactly once with the existing fresh Checkout. Do not create another Checkout for this repair.
