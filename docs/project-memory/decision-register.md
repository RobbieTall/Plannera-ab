...
### Decision: diagnostic refinement is not acceptance

Expose only fixed configuration categories; preserve all refusal checks and read-only behavior. Do not infer secret contents from a masked value or generic failure. Independent AI review is not the required human environment approval. Actual candidate/check/review/run evidence is recorded in Issue #395 before promotion.
