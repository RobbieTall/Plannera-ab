...
A later DCP Deep Dive purchase, when implemented, must bind to the claimed project, exact site snapshot, and proposal intent. This slice does not implement price, Stripe, quotas, credits, checkout, entitlements, auth policy, PWA manifest/service worker, consultant sending, or the promotion gate.
