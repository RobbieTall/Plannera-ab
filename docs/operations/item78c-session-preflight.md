...
Both environment reviewer gates were observed OFF and administrator bypass ON on 23 September 2026. The workflow intentionally refuses those settings. Do not weaken the checks to accommodate them. Preserve the existing exact branch restriction when strengthening controls.
...
`node --test tests/item78c-session-preflight.test.mjs tests/item78c-session-preflight-authorize.test.mjs`
