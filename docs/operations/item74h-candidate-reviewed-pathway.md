...
- Stamped-plan page 2 has not been retained and operator-reviewed, so shed height and elevations remain unconfirmed.
...
- Gate 04 numeric envelope: MORE_EVIDENCE until height and legal set-out are confirmed. Any variation requiring justification routes to MERIT.
...
- A$49 Planning Controls Pack: available as a working exact-scope pack. It includes confirmed instrument, R2 proposal zone, parcel, approval-pathway, area, depicted-dimension and provenance evidence. It visibly excludes legal setback certification, height and unreviewed determination conditions.
...
The next Preview run should retain stamped-plan page 2 only after the same exact public source is revalidated, scanned in the deny-network sandbox and proven zero residue. The operator should then review height and elevations and review the determination conditions.
