...
export default async function ProjectWorkspacePage({ params }: WorkspacePageProps) {
  const { sessionId, userId } = await getSessionContext();

  const project = await getProjectForRequester(params.id, sessionId, userId);
  ...
  return <ProjectWorkspace project={workspaceProject} userId={userId} />;
}
