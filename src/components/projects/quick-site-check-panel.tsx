...
export function QuickSiteCheckPanel({
  ...
  const siteLabel =
    report?.site.address ?? siteContext?.formattedAddress ?? siteContext?.addressInput ?? "No address";
  ...
  return (
    <>
      ...
      >
        ...
        <p className="text-xs text-slate-500 dark:text-slate-300">Zoning + mapped controls snapshot.</p>
      </button>
      ...
      <Modal
        ...
        description="LEP-only summary of zoning and mapped controls."
        size="lg"
      >
        ...
        {!hasSite ? (
          <div className="flex items-center gap-2 rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-4 text-sm text-slate-500 dark:border-slate-700 dark:bg-slate-800/60 dark:text-slate-300">
        ...
        {status === "error" && error ? (
          <div className="rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700 dark:border-rose-900/60 dark:bg-rose-950/40 dark:text-rose-100">
            {error}
          </div>
        ) : null}

        {status === "loading" ? (
          <p className="text-sm text-slate-500 dark:text-slate-300">Running Quick Site Check…</p>
        ) : null}

        {!report && status === "idle" && hasSite ? (
          <p className="text-sm text-slate-500 dark:text-slate-300">Run Quick Site Check to view zoning and mapped controls.</p>
        ) : null}
          ...
          <div className="space-y-4">
            <div className="rounded-2xl border border-slate-200 bg-slate-50/80 p-4 dark:border-slate-800 dark:bg-slate-800/60">
                  ...
                  {saveMessage ? (
                    <p
                      className={cn(
                        ...
                        saveStatus === "success" ? "text-emerald-600 dark:text-emerald-300" : "text-rose-600 dark:text-rose-300",
                      )}
                    >
                      {saveMessage}
                    </p>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2 text-sm text-slate-700 dark:text-slate-100">
                ...
                <span className="text-xs text-slate-400 dark:text-slate-500">Generated {formatShortDate(report.generatedAt)}</span>
              </div>
