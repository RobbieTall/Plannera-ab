...
const siteContext = { formattedAddress: project.address, lgaName: "Byron Shire", lgaCode: "BYRON", zone: "SP3 Tourist" };
const date = (s: string) => new Date(s);
const row = (id: string, type: string, payload: any, generatedAt: string) => ({ id, type, payload, projectId: "proj", title: id, capturedAt: date(generatedAt), createdAt: date(generatedAt) });
const qsc = (id = "qsc", overrides: any = {}, generatedAt = "2026-07-15T00:00:00Z") => row(id, "quick_site_check", {
...
const dpp = (id = "dpp", sourceQsc = "qsc", commercialReady = true, generatedAt = "2026-07-15T00:10:00Z", overrides: any = {}) => row(id, "detailed_planning_pack", {
  packType: "detailed_planning_pack", generatedAt, projectId: "proj", site: { address: project.address, lga: "Byron Shire", lgaCode: "BYRON", zoneCode: "SP3", zoneName: "Tourist", zoneLabel: "SP3 Tourist" }, proposalBrief: "shop fitout",
  sourceQuickSiteCheck: { artefactId: sourceQsc, title: "QSC", generatedAt: null, lepEvidenceSummary: null }, carriedLepEvidenceSummary: null,
...
const audit = (artefacts: any[]) => auditCommercialFunnel("pub", { prisma: prismaFor(artefacts) });

describe("auditCommercialFunnel", () => {
  it("returns unknown project honestly", async () => expect(await auditCommercialFunnel("missing", { prisma: prismaFor([], null) })).toMatchObject({ project: null, error: "project_not_found" }));
  it("is read-only and reports Byron quality chain with exact IDs", async () => expect(await audit([qsc(), dpp(), see()])).toMatchObject({ quickSiteCheck: { artefactId: "qsc", state: "ready" }, detailedPlanningPack: { artefactId: "dpp", sourceQuickSiteCheckArtefactId: "qsc", state: "ready" }, see: { artefactId: "see", sourceDetailedPlanningPackArtefactId: "dpp", sourceQuickSiteCheckArtefactId: "qsc", state: "ready" }, referralEligibility: "quality_chain_referral" }));
  it("accepts a legitimate publicId QSC report projectId in the exact chain", async () => expect(await audit([qsc("qsc-public", { projectId: "pub" }), dpp("dpp-public", "qsc-public"), see("see-public", "dpp-public", "qsc-public")])).toMatchObject({ quickSiteCheck: { artefactId: "qsc-public", state: "ready" }, detailedPlanningPack: { artefactId: "dpp-public", sourceQuickSiteCheckArtefactId: "qsc-public", state: "ready" }, referralEligibility: "quality_chain_referral" }));
  it("rejects a forged unrelated QSC report projectId", async () => expect(await audit([qsc("forged-qsc", { projectId: "evil" }), dpp("dpp-forged-qsc", "forged-qsc")])).toMatchObject({ detailedPlanningPack: { state: "stale_mismatched_malformed" }, referralEligibility: "none" }));
  it("accepts Kempsey E2 D4 nil/0m evidence without conflicting-scope upgrade", async () => { const kProject = { ...project, address: "52 Belgrave St, Kempsey NSW 2440", zoning: "E2 Commercial Centre", zoningCode: "E2" }; const kSite = { formattedAddress: kProject.address, lgaName: "Kempsey Shire", lgaCode: "KEMPSEY", zone: "E2 Commercial Centre" }; const kScope = { address: kProject.address, lga: "Kempsey Shire", lgaCode: "KEMPSEY", zoneCode: "E2", zoneName: "Commercial Centre", zoneLabel: "E2 Commercial Centre" }; const rows = [qsc("kq", { site: kScope }), dpp("kd", "kq", true, "2026-07-15T00:10:00Z", { site: kScope }), see("ks", "kd", "kq", "2026-07-15T00:20:00Z", { siteDescription: { address: kProject.address, lga: "Kempsey Shire", zoneCode: "E2", zoneName: "Commercial Centre", zoneLabel: "E2 Commercial Centre" } })]; const p = prismaFor(rows, kProject); p.project.findUnique = async () => ({ ...kProject, siteContext: kSite }); const result = await auditCommercialFunnel("pub", { prisma: p }); expect(result).toMatchObject({ referralEligibility: "quality_chain_referral", detailedPlanningPack: { citedTopicCount: 1 } }); });
  it("uses newest active unresolved DPP over older ready and omits SEE readiness", async () => expect(await audit([qsc(), dpp("ready", "qsc", true, "2026-07-15T00:10:00Z"), see("old-see", "ready", "qsc"), dpp("unresolved", "qsc", false, "2026-07-15T00:30:00Z")])).toMatchObject({ detailedPlanningPack: { artefactId: "unresolved", state: "needs_expert_review" }, see: { state: "stale_mismatched_legacy" }, referralEligibility: "unresolved_pack_referral" }));
  it("anchors QSC section to selected DPP source despite newer unrelated cited QSC", async () => expect(await audit([qsc("source", {}, "2026-07-15T00:00:00Z"), qsc("newer", {}, "2026-07-15T00:50:00Z"), dpp("dpp", "source"), see("see", "dpp", "source")])).toMatchObject({ quickSiteCheck: { artefactId: "source" }, detailedPlanningPack: { sourceQuickSiteCheckArtefactId: "source" } }));
