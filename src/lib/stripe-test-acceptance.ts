...
const postHeaders = (cookie: string) => ({ "content-type": "application/json", cookie });
...
export function validateAcceptanceConfiguration(env: AcceptanceEnv) {
  ...
  return { phase, baseUrl, stripeKey, sessionId, cookie, projectId, proposal, otherProjectId, otherProposal, quickSiteCheckArtefactId, dppBody: record };
}
...
const appPost = async (fetcher: typeof fetch, url: URL, cookie: string, body: unknown): Promise<Response> => {
  return fetcher(url, { method: "POST", redirect: "error", headers: postHeaders(cookie), body: JSON.stringify(body) }).catch(() => fail("application_request_failed"));
};
const status = async (fetcher: typeof fetch, baseUrl: URL, cookie: string, body: unknown): Promise<PlanningPackEnabledStatusResponse> => {
  const response = await appPost(fetcher, new URL("/api/planning-pack/status", baseUrl), cookie, body);
...
const countExactAcceptanceDpps = async (fetcher: typeof fetch, baseUrl: URL, cookie: string, projectId: string, proposal: string, quickSiteCheckArtefactId: string) => {
  const url = new URL(`/api/projects/${encodeURIComponent(projectId)}/artefacts`, baseUrl);
  const response = await fetcher(url, { method: "GET", redirect: "error", headers: { cookie } }).catch(() => fail("application_request_failed"));
  if (!response.ok) fail("application_request_failed");
  let value: unknown;
...
export async function runStripeTestAcceptance(env: AcceptanceEnv, fetcher: typeof fetch): Promise<{ exitCode: number; summary: SafeAcceptanceSummary }> {
  ...
  try {
    ...
    const exact = await status(fetcher, config.baseUrl, config.cookie, exactBody);
    const changedProposal = await status(fetcher, config.baseUrl, config.cookie, otherProposalBody);
    const otherProject = await status(fetcher, config.baseUrl, config.cookie, otherProjectBody);
    ...
    if (phase === "paid") {
      for (let attempt = 0; attempt < 2; attempt++) { const response = await appPost(fetcher, new URL("/api/planning-pack/checkout", config.baseUrl), config.cookie, exactBody); if (response.status !== 409) fail("checkout_replay_failed"); }
    ...
    const dppCountBefore = await countExactAcceptanceDpps(fetcher, config.baseUrl, config.cookie, config.projectId, config.proposal, config.quickSiteCheckArtefactId);
    ...
    if (phase === "paid" && dppCountBefore === 0) {
      if ((await appPost(fetcher, dppUrl, config.cookie, exactBody)).status !== 201) fail("dpp_gate_failed");
      checks.dppCreatedThisRun = true;
    } else if (phase !== "paid" && (await appPost(fetcher, dppUrl, config.cookie, exactBody)).status !== 402) fail("dpp_gate_failed");
    if (phase === "paid") for (const body of [otherProposalBody, otherProjectBody]) { if ((await appPost(fetcher, dppUrl, config.cookie, body)).status !== 402) fail("dpp_gate_failed"); }
    const dppCountAfter = await countExactAcceptanceDpps(fetcher, config.baseUrl, config.cookie, config.projectId, config.proposal, config.quickSiteCheckArtefactId);
